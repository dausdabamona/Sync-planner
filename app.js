...
// DIURUTKAN: dari target paling sedikit & text pendek → target paling banyak & text panjang
const DZIKIR_DATA = [
...
// Increment dzikir fullscreen - DENGAN AUTO-ADVANCE
function incrementDzikirFs() {
  if (!dzikirState.currentDzikir) return;
  
  const target = dzikirState.currentDzikir.target;
  const prevCount = dzikirState.currentCount;
  
  dzikirState.currentCount++;
  document.getElementById('dzikirFsCount').textContent = dzikirState.currentCount;
  
  // Update progress di state
  if (!dzikirState.progress[dzikirState.time]) {
    dzikirState.progress[dzikirState.time] = {};
  }
  dzikirState.progress[dzikirState.time][dzikirState.currentDzikir.id] = dzikirState.currentCount;
  
  updateDzikirFsProgress();
  
  // Check if target reached
  if (prevCount < target && dzikirState.currentCount >= target) {
    // TARGET TERCAPAI! Getaran panjang
    if (navigator.vibrate) {
      navigator.vibrate([100, 50, 100, 50, 200]); // Pola getaran panjang
    }
    
    // Simpan progress
    saveDzikirProgress();
    
    // Auto advance ke dzikir berikutnya setelah delay
    setTimeout(() => {
      autoAdvanceToNextDzikir();
    }, 800); // Tunggu 800ms agar user merasakan getaran
...
// Auto advance ke dzikir berikutnya
function autoAdvanceToNextDzikir() {
  // Cek apakah masih ada dzikir berikutnya
  if (dzikirState.currentIndex >= DZIKIR_DATA.length - 1) {
    // Sudah dzikir terakhir
    showToast('Masya Allah! Semua dzikir selesai ✨', 'success');
    return;
  }
  
  // Pindah ke dzikir berikutnya
  dzikirState.currentIndex++;
  const dzikir = DZIKIR_DATA[dzikirState.currentIndex];
  dzikirState.currentDzikir = dzikir;
  
  // Load progress untuk dzikir baru (atau mulai dari 0)
  const progress = dzikirState.progress[dzikirState.time] || {};
  dzikirState.currentCount = progress[dzikir.id] || 0;
  ...
  console.log('[Dzikir] Auto-advance ke:', dzikirState.currentIndex, dzikir.id);
}
...
// Mulai sesi dzikir dari dzikir pertama yang belum selesai
function startDzikirSession() {
  const progress = dzikirState.progress[dzikirState.time] || {};
  
  // Cari dzikir pertama yang belum selesai
  const firstIncompleteIndex = DZIKIR_DATA.findIndex(d => {
    const count = progress[d.id] || 0;
  ...
  // Buka dzikir yang belum selesai
  openDzikirCounter(DZIKIR_DATA[firstIncompleteIndex].id);
}
